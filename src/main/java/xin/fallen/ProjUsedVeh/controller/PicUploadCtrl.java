...
import xin.fallen.ProjUsedVeh.util.FtpUtil;
...
public class PicUploadCtrl {
    ...
    public JsonResult newPicUpload(HttpServletRequest req, HttpServletResponse resp) {
        ...
        String values = null;
        try {
            values = IOUtils.toString(req.getInputStream(),"UTF-8");
        } catch (IOException e) {
        ...
        try {
            ...
            return JsonResultUtil.resDispatcher("上传失败(图片生成失败)", 0);
        }

        boolean flag = FtpUtil.upload(ftpClientPool, ftpConfig, file, StaticConfig.FtpUploadPath + StaticConfig.FtpFilenamePattern.replace("{FILENAME}", file.getName()));
        if (!flag) {
            try {
            ...
            return JsonResultUtil.resDispatcher("FTP上传失败", 0);
        }
